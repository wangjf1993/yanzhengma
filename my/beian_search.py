...
MONGODB_DOCNAME = 'chongqing_frv_test'
MONGODB_USER = 'gouuse'
...
def get_redis_company():
    ...
    redis_ = Redis(host=REDIS_HOST, port=REDIS_PORT, db=REDIS_DB,
                   password=REDIS_PASSWORD)
    comapny_name = redis_.spop('chongqing_company_2').decode()
    return comapny_name
...
def beian_spider(company_name, proxies):
    company_item = {}
    while True:
    ...
    return company_website, proxies
...
def main():
    while  True:
        company_name_1 = get_redis_company()
        t=company_name_1.split('|')

        company_name=t[0].replace('www.','')
        ...
        beian_item = beian_spider(company_name, proxies)
        website_beian, proxies = beian_item
