...
def main():
    ...
    test_sample_imge=sample_conf["test_image_dir"]

    img_list = os.listdir(test_sample_imge)
    # file_paths = os.path.join(origin_dir, img_name)
    my_list=[]
    for index, img_name in enumerate(img_list):
        item={}
        r_img = Image.open(f"./sample/test/{img_name}")
        t = R.rec_image(r_img)
        print('预测结果%s' % t, '       s实际图片%s' % img_name)
        item['tupianming'] = img_name
